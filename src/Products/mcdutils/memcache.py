...
See U{the MemCached homepage<http://www.danga.com/memcached>} for more about
memcached.
...
This should give you a feel for how this module operates::
    ...
    mc.set('key', '1')   # the key used for incr/decr must be a string.
    mc.incr('key')
    mc.decr('key')
...
class Client:
    ...
    def set_servers(self, servers):
        ...
        Servers can be passed in two forms:
            1. Strings of the form C{"host:port"} imply a default weight of 1.
    ...
    def incr(self, key, delta=1):
        ...
        C{key} by C{delta}, or by 1 if C{delta} is unspecified.  Returns None
        if C{key} doesn't exist on server, otherwise it returns the new value
        after incrementing.
        ...
        @param delta: Integer amount to increment by (zero or greater).
    ...
    def decr(self, key, delta=1):
        ...
        @param delta: Integer amount to decrement by (zero or greater).
    ...
    def add(self, key, val, time=0):
        ...
        Like L{set}, but only stores in memcache if key doesn't already exist.
    ...
    def set(self, key, val, time=0):
        ...
        value.
        You may prefer, for example, to keep all of a given user's objects on
        the same memcache server, so you could use the user's unique id as the
        hash value.
    ...
    def get_multi(self, keys):
        ...
        This method is recommended over regular L{get}, it lowers the number of
        total packets flying around your network, reducing total latency, since
        your app doesn't have to wait for each round-trip of L{get} before
    ...
    def _recv_value(self, server, flags, rlen):
        ...
        if len(buf) != rlen:
            raise _Error('received %d bytes when expecting %d' % (len(buf),
                                                                  rlen))
...
class _Host:
    ...
    def expect(self, text):
        line = self.readline()
        if line != text:
            msg = "while expecting '%s', got unexpected response '%s'"
            self.debuglog(msg % (text, line))
        return line
